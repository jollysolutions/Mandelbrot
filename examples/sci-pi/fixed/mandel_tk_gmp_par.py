...
# MJR 2/2021  http://www.sci-pi.org.uk/maths/mandel_tk_gmp_par.html

# Licence: GPL v3
#
# Add multiprocesing
#
# Fork as fixed point integer version using GMP
#
...
def mandel_column(x_in,ymin,ymax,size,maxiter):
    ...
    x0=mpz(x_in)
    y1=mpz(ymax)
    mod2_test=mpz(4)<<(2*bshift)
    bshiftm1=bshift-1
    for j in range(0,size):
        y0=y1-int((j*(ymax-ymin))/size)
        i=0
        x=x0
        y=y0
        x2=x*x
        ...
        for i in range(0,maxiter):
            if (x2+y2>=mod2_test): break
            y=((x*y)>>bshiftm1)+y0
            x=((x2-y2)>>bshift)+x0
            x2=x*x
            y2=y*y
#            x=int((x*x-y*y)/2**bshift+x0)
#            y=int((2*tmp*y)/2**bshift+y0)
            i=i+1
        col[j]=1-i/maxiter
