...
# Mandelbrot set using tkinter, numba, and little else, save
# matplotlib for colour maps
# MJR 2/2021  http://www.sci-pi.org.uk/maths/mandel_tk.html

# Licence: GPL v3

# Jul 2022, change names of modified colormaps for compatibility with
#           matplotlib 3.5
...
#   -- tolerate new lines in co-ordinate strings
#
...
@jit(nogil=True)
def mandel_column(x,ymin,ymax,size,maxiter):
    col=np.zeros(size)
    ystep=(ymax-ymin)/size
    for j in range(0,size):
        z0=complex(x,ymax-j*ystep)
        i=0
        z=z0
...
def finished_calc(t,maxiter,size):
    ...
    legend.config(text="Finished in %.2fs, maxiter=%d, res=%d nproc=%d"%
                  (t,maxiter,size,nproc))
    ppm_header=b"P6\n%i %i\n255\n"%(size,size)
    if (False):
...
def recalculate(old_coords=[]):
    ...
    if (new_xmax-new_xmin)>0:
        ...
        ymin=new_ymin
        ymax=new_ymax
    history.append((xmin,xmax,ymin,ymax,size,maxiter))
...
#                           ("%+.10f"%(ymax)).rstrip("0")+"i)")
    coords_string=(("(%.12f"%(xmin)).rstrip("0")+
                        ("%+.12f"%(ymin)).rstrip("0")+
                        ("i) to (%.12f"%(xmax)).rstrip("0")+
                        ("%+.12f"%(ymax)).rstrip("0")+"i)")
...
def cursor_update(event):
    ...
    cursor.config(text="% .10f%+.10fi "%
                    (xmin+x*((xmax-xmin)/(zoom*size)),
                     ymax-y*((ymax-ymin)/(zoom*size))))
...
# could simply use size*zoom, but the below is more general
            try:
                ...
                xsize=int(self.widget['width'])
                ysize=xsize
            new_xmin=xmin+(xmax-xmin)*(x/xsize)
            new_ymin=ymax-(ymax-ymin)*((y+h)/ysize)
            new_xmax=xmin+(xmax-xmin)*((x+w)/xsize)
            new_ymax=ymax-(ymax-ymin)*(y/ysize)
        else:
            self.widget.delete(self.band)
            self.visible=False
...
def back():
    ...
    history.pop()
    (new_xmin,new_xmax,new_ymin,new_ymax,size,maxiter)=history.pop()
    maxiter_entry.delete(0,last=tk.END)
    maxiter_entry.insert(tk.END,"%d"%(maxiter))
    if (((new_xmin<=xmin) and (new_ymin<=ymin) and
...
class New_coords():
    ...
    def __init__(self,main):
        ...
        xmin_entry.insert(tk.END,("%.14f"%xmin).rstrip("0"))
        ...
        xmax_entry.insert(tk.END,("%.14f"%xmax).rstrip("0"))
        ...
        ymin_entry.insert(tk.END,("%.14f"%ymin).rstrip("0"))
        ...
        ymax_entry.insert(tk.END,("%.14f"%ymax).rstrip("0"))
